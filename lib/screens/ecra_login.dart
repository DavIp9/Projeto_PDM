...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _login() async {
    ...
    try {
      ...
      var user =
          await FirestoreRepository.instance.obterUtilizadorPorEmail(email);

// Repara automaticamente contas antigas que existem no Authentication,
// mas ainda não possuem documento no Cloud Firestore.
      if (user == null) {
        final nomeFirebase = firebaseUser.displayName?.trim();

        final nome = nomeFirebase != null && nomeFirebase.isNotEmpty
            ? nomeFirebase
            : email.split('@').first;
        ...
        user = await FirestoreRepository.instance.obterUtilizadorAtual();
      }
      ...
      if (user == null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text(
              'Este utilizador não está registado na aplicação.',
            ),
          ),
        );
        return;
...
// Restantes logins: entra diretamente na aplicação.
      ...
      if (e.code == 'invalid-email') {
        ...
        mensagem = 'Erro ao iniciar sessão.';
      }
