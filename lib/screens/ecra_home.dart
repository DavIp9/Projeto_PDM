...
class _MainDashboardScreenState extends State<MainDashboardScreen> {
  ...
  Future<void> _loadStats() async {
    try {
      ...
      final db = await DatabaseHelper.instance.database;
      final unreadCountRes = await db.rawQuery(
        'SELECT COUNT(*) as unreadCount FROM NOTIFICACOES WHERE ID_UTILIZADOR = ? AND FASE != ?',
        [user.idUtilizador, 'Lida'],
      );
      final unreadCount = (unreadCountRes.first['unreadCount'] as int?) ?? 0;
                                          ...
                                          child: GestureDetector(
                                            onTap: () {
                                              Navigator.push(
                                                context,
                                                MaterialPageRoute(
                                                  builder: (_) => DetalhesBadgeScreen(
                                                    badge: Map<String, dynamic>.from(rec),
                                                  ),
                                                ),
                                              ).then((_) => _loadStats());
                                            },
  ...
  Future<void> _verificarMarcosCelebracao(int totalPontos, int totalBadges) async {
    ...
    for (var milestone in badgeMilestones) {
      final key = 'celebrated_badge_$milestone';
    ...
    for (var milestone in pointsMilestones) {
      final key = 'celebrated_points_$milestone';
      final alreadyCelebrated = prefs.getBool(key) ?? false;
  ...
  void _mostrarPopupCelebracao(String title, String message, String type) {
    showDialog(
      ...
      builder: (BuildContext context) {
        final iconData = type == 'Trophy' ? Icons.emoji_events_rounded : Icons.offline_bolt_rounded;
